...
namespace NerdStore.Catalogo.Data.Repository
{
    public class ProdutoRepository : IProdutoRepository
    {
        ...
        public async Task<Produto> ObterPorId(Guid id) => await _context.Produtos.AsNoTracking().FirstOrDefaultAsync(p => p.Id == id);

        public async Task<IEnumerable<Produto>> ObterPorCategoria(int codigo) => await _context.Produtos.AsNoTracking().Include(p => p.Categoria).Where(c => c.Categoria.Codigo == codigo).AsNoTracking().ToListAsync();
        ...
        {
            var local = _context.Set<Produto>().Local.Where(_ => _.Id == produto.Id).FirstOrDefault();
            if (local != null)
